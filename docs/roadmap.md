...
## Version 2 — Optimization (code built; GPU performance experiments pending)
...
## Version 3 — A Triton kernel (code built; CUDA verification passed)
...
CUDA correctness tests passed on a Runpod RTX 4090 with PyTorch 2.8.0+cu128 and Triton 3.4.0. Follow the [Version 3 walkthrough](version-3.md). Remaining work is to save repeated GPU kernel timing measurements.
...
## Version 4 — KV cache (code built; CUDA parity passed)
...
Cached CUDA parity tests passed on the Runpod RTX 4090 for small GPT-2 and Llama-style models. Remaining work is to run repeated pretrained GPU experiments across prompt lengths of 128, 256, 512, 1024, and 2048 tokens.
...
The profiler command is tested on CPU. NVIDIA Nsight capture commands are documented but have not been executed yet. Run profiling separately from benchmarks, then repeat the comparisons to check that the conclusions hold.
